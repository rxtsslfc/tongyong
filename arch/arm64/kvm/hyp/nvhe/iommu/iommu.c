...
void *kvm_iommu_donate_pages(u8 order, bool request)
{
	...
	return __kvm_iommu_donate_pages(pool, order, request);
}
...
int kvm_iommu_alloc_domain(pkvm_handle_t domain_id, u32 type)
{
	 ...
	 * Guest ID space manage by the hypervisor, so it is trusted.
	 */
	if (!ctxt && (domain_id >= (KVM_IOMMU_MAX_DOMAINS >> 1)))
		return -EINVAL;
...
out_unlock:
	pkvm_devices_iommu_unlock(iommu_id, endpoint_id);
	return ret;
}
